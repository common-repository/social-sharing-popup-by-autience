...
angular.module('yetienceApp')
    .controller('extensionSaleController', ['$scope', 'CommService', 'SettingsService', '$rootScope', 'localStorageService', function($scope, CommService, SettingsService, $rootScope, localStorageService) {
        ...
        $scope.priceForFeature = function(label) {
            // console.log('label is',label)
            var feature_id_price = label + '_price'
                // console.log(feature_id_price)
            var price = localStorageService.get(feature_id_price)
                // console.log(price)
            if (!price) {
                //if price is not set, generate a random num between 0-8, add 3 and floor it. this is the new price
                var generated_price = randomInteger(3,9);//Math.floor(3 + (8 * Math.random()));
                price = localStorageService.set(feature_id_price, generated_price);
...
*/
        ...
        $scope.feedbackComplete = function(feedback) {
            switch (feedback.working) {
                case "yes":
                    return (feedback.freeAdvice == 'yes' && feedback.email && feedback.email.length > 3)
                case "no":
...
angular.module('yetienceApp')
    .directive('componentForm', ['$rootScope', function($rootScope) {
        // Runs during compile
        return {
            controller: ['$scope', '$element', 'CommService', '$modal', '$stateParams', '$rootScope', function($scope, $element, CommService, $modal, $stateParams, $rootScope) {
                $scope.cust = {
                    fields: [],
                    values: {}
                }
        ...
        return {
            controller: ['$rootScope', '$element', '$scope', 'SettingsService', 'CommService', function($rootScope, $element, $scope, SettingsService, CommService) {
                ...
                function fetchAndAssignLists(provider) {
                    ...
                    if (provider && provider.indexOf('Yeloni') < 0) {
    ...
    .service('SettingsService', ['CommService', '$rootScope', 'configurationFields', '$cookies', '$q', 'UtilsService', '$stateParams', '$location', function(CommService, $rootScope, configurationFields, $cookies, $q, UtilsService, $stateParams, $location) {
        ...
        function getWebsiteAndAttach(website_id, check_saved) {
            ...
            return CommService.getWebsite(website_id)
                .then(function(website) {
                    //adding feature to website
                    // var extensions = {}
                    // extensions.mobileScreens = 1466208000
                    // extensions.specificPages = 1466208000
                    // if (website)
        ...
        function addAdvancedConfigurationOnSetup(setup, website) {
            ...
            if (setup.widgets && website.extensions) {
            ...
            //adding advancedConfiguration for old premium users
        ...
        this.hasFeature = function(feature) {
            if (setup) {
                ...
                if (setup.extensions && setup.extensions[feature]) {
                    return true
                }
            }
...
angular.module('yetienceApp')
    .constant('configurationFields', {
        ...
        feedback: [{
            className: "section-label",
            template: "<strong>How is your Popup working?</strong> <br><br>Open any page of your website and check if your popup is working as expected",
            ...
            type: "radio",
            templateOptions: {
                label: "Would you like to receive notifications on bug fixes and feature updates?",
                labelProp: "field",
                valueProp: "value",
                options: [{
                    field: "Yes",
